...
async function collectTestSnapshot(
  ...
  try {
    ...
    // Fetch detailed data
    const employeeData = await fetchEmployeeDetails(apiKey, testEmployee.id);
    
    // Store snapshot
    await storeSnapshot(supabase, session.id, testEmployee.id, employeeData);
...
async function collectSpecificSnapshots(
  ...
  for (const employeeId of employeeIds) {
    try {
      ...
      // Fetch detailed data
      const employeeData = await fetchEmployeeDetails(apiKey, employeeId);
      
      // Store snapshot
      await storeSnapshot(supabase, session.id, employeeId, employeeData);
  ...
  try {
    ...
    for (const employee of employees) {
      try {
        ...
        // Fetch detailed data
        const employeeData = await fetchEmployeeDetails(apiKey, employee.id);
        
        // Store snapshot
        await storeSnapshot(supabase, session.id, employee.id, employeeData);
...
async function fetchEmployeeDetails(apiKey: string, employeeId: string): Promise<any> {
...
async function storeSnapshot(
  ...
  const record: TemporalRecord = {
    ...
    data_hash: JSON.stringify(apiResponse),
    collected_at: now,
    effective_from: now,
    effective_to: null,
    is_latest: true,
  ...
  if (updateError) {
    console.log('Note: No previous data to update for ' + employeeId);
  }
