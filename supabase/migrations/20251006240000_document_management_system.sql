...
BEGIN
  RETURN QUERY
  SELECT
    COUNT(*) FILTER (WHERE dt.is_required)::integer AS total_required,
    COUNT(*) FILTER (WHERE dt.is_required AND sd.status = 'uploaded' AND sd.is_current = true)::integer AS uploaded_count,
    COUNT(*) FILTER (WHERE dt.is_required AND (sd.status = 'missing' OR sd.id IS NULL))::integer AS missing_count,
    COUNT(*) FILTER (WHERE sd.status = 'expired')::integer AS expired_count,
    COUNT(*) FILTER (WHERE sd.status = 'uploaded' AND sd.is_current = true AND sd.expires_at IS NOT NULL AND sd.expires_at <= CURRENT_DATE + INTERVAL '30 days')::integer AS expiring_soon_count
...
-- Staff Documents: Simplified RLS - Allow authenticated users to manage documents
-- Note: Application-level security handles staff-specific access
...
-- Note: Commented out because this should be done via the RPC function
-- or when staff profiles are loaded in the application
