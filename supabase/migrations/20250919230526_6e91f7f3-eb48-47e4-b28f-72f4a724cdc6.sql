...
ALTER TABLE emails ADD COLUMN IF NOT EXISTS email_type text DEFAULT 'received' CHECK (email_type IN ('received', 'sent', 'draft'));

-- Add email status sync columns
ALTER TABLE emails ADD COLUMN IF NOT EXISTS gmail_label_ids text[] DEFAULT '{}';
...
CREATE INDEX IF NOT EXISTS idx_emails_body_search ON emails USING gin(to_tsvector('english', coalesce(body_text, '')));
