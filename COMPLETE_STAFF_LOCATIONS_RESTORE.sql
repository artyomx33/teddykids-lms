-- =========================================
-- 🏢 COMPLETE STAFF LOCATIONS RESTORATION
-- =========================================
...
-- Step 7: Verification and results
SELECT
  '🎉 STAFF LOCATIONS RESTORED!' as status,
  location_key,
  manager,
...
-- Show sample staff for each location
SELECT
  'Sample staff by location:' as info,
  location_key,
  manager,
  array_agg(full_name ORDER BY full_name) FILTER (WHERE full_name IS NOT NULL) as sample_names
FROM public.staff
WHERE location_key IS NOT NULL
GROUP BY location_key, manager
ORDER BY location_key;
...
SELECT
  ...
  COUNT(*) FILTER (WHERE location_key IS NOT NULL) as mapped_to_locations,
  COUNT(*) FILTER (WHERE manager IS NOT NULL) as with_managers,
